...
class GPT(nn.Module):
    ...
    def forward(self, idx, targets=None):
        ...
        con_tokens=idx[:,1:self.con_num+1]
        idx=torch.cat((idx[:,0].unsqueeze(1),idx[:,self.con_num+1:]),1)
        if targets is not None:
            targets=targets[:,self.con_num:]
        b, t = idx.size()
        assert t <= self.block_size, "Cannot forward, model block size is exhausted."

        token_embeddings = (self.tok_emb(idx)+self.tok_emb(con_tokens).sum(dim=1).unsqueeze(1))* math.sqrt(self.d_model) 
